...
export default function WeeklyLinksManager({ weekId, links, onAddLink, onRemoveLink }) {
  ...
  const handleSubmit = (e) => {
    ...
    // Basit bir URL doğrulama ve düzeltme (http yoksa ekleme)
    let finalUrl = url.trim();
    if (!/^https?:\/\//i.test(finalUrl)) {
      finalUrl = 'https://' + finalUrl;
    }

    onAddLink(weekId, { id: Date.now().toString(), title: title.trim(), url: finalUrl });
    setTitle('');
    setUrl('');
  };
      ...
      <div className="p-4 min-h-[100px] flex flex-col gap-2 bg-slate-900/50">
        {links && links.length > 0 ? (
          ...
          <div className="text-center py-6 text-slate-500 text-sm italic">
            Bu haftaya henüz bir kaynak eklenmedi. Öğretici videolar veya makaleler ekleyerek başlayabilirsiniz...
          </div>
        )}
      </div>
    </div>
